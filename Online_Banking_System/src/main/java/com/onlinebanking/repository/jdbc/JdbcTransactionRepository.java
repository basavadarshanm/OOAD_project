...
public class JdbcTransactionRepository implements TransactionRepository {
    ...
    @Override
    public Transaction create(long accountId, String type, BigDecimal amount, LocalDateTime occurredAt, String description) {
        String sql = "INSERT INTO transactions (account_id, type, amount, occurred_at, description) VALUES (?,?,?,?,?)";
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    return new Transaction(id, accountId, type, amount, occurredAt, description);
                }
            }
            throw new IllegalStateException("Failed to retrieve generated transaction ID");
        } catch (Exception e) {
